...
export default function WeekCalendar() {
  ...
  return (
    <Box className="bg-zinc-50 rounded-[20px] shadow-sm" sx={{ p: 2, height: '100%', display: 'flex', flexDirection: 'column' }} onClick={() => navigate('/calendario')}>
      ...
      {/* Scrollable calendar area (horizontal on small screens) */}
      <Box sx={{ flex: 1, minHeight: 0, overflowX: 'auto', display: 'flex', flexDirection: 'column' }}>
        {/* Column headers — fijos */}
        <Box sx={{ display: 'grid', gridTemplateColumns: '48px repeat(6, 1fr)', flexShrink: 0, minWidth: 420 }}>
          <Box />
          {weekDays.map((d, i) => {
            const isToday =
        ...
        <Box sx={{ flex: 1, minHeight: 0, overflowY: 'auto', minWidth: 420 }}>
